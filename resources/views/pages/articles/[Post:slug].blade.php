<x-app-layout>
    <div class="relative">
        <div class="mx-auto max-w-3xl">
            <a  href="/" wire:navigate aria-label="Go back to articles" class="group mb-8 flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-md shadow-zinc-800/5 ring-1 ring-zinc-900/5 transition dark:border dark:border-zinc-700/50 dark:bg-zinc-800 dark:ring-0 dark:ring-white/10 dark:hover:border-zinc-700 dark:hover:ring-white/20 lg:absolute lg:-left-5 lg:-mt-2 lg:mb-0 xl:-top-1.5 xl:left-0 xl:mt-0">
                ...
                <header class="flex flex-col">
                    ...
                    <div class="flex space-x-5 mt-5">
                        @foreach($post->categories as $category)
                            <a wire:navigate href="/category/{{ $category->slug }}" class="inline-block mb-2 text-xs font-semibold tracking-wide text-blue-500 uppercase transition dark:text-teal-400 hover:text-teal-600 dark:hover:text-teal-500">{{ $category->name }}</a>
                        @endforeach
                    </div>
