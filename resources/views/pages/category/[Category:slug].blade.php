@php
    $posts = \App\Models\Post::where('status', 'published')->whereHas('categories', function ($query) use ($category) {
...
<x-app-layout>
    <div class="flex max-w-3xl flex-col space-y-16 mx-auto">
        <h1 class="font-extrabold text-2xl">{{ $category->name }}</h1>
            ...
            <article class="md:grid md:grid-cols-4 md:items-baseline">
                <div class="md:col-span-3 group relative flex flex-col items-start">
                    <h2 class="text-base font-semibold tracking-tight text-zinc-800 dark:text-zinc-100">
                        ...
                        <a wire:navigate href="/articles/{{ $post->slug }}">
                    ...
                    <div aria-hidden="true" class="relative z-10 mt-4 flex items-center text-sm font-medium text-blue-500">Read article<svg viewBox="0 0 16 16" fill="none" aria-hidden="true" class="ml-1 h-4 w-4 stroke-current"><path d="M6.75 5.75 9.25 8l-2.5 2.25" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></div>
                </div>
                <time class="mt-1 hidden md:block relative z-10 order-first mb-3 flex items-center text-sm text-zinc-400 dark:text-zinc-500" datetime="2022-09-05">{{ $post->published_at->format('F d, Y') }}</time>
            </article>
        @endforeach
    </div>
</x-app-layout>
